...
- 导入 HTML：导入浏览器标准导出的 Netscape HTML 书签文件，并保留文件夹层级`n- URL 去重：添加或导入时跳过目标文件夹中已存在的相同网址`n- 实时刷新：书签在其他页面或窗口发生变化时自动更新列表
...
- `popup.html`：弹出窗口界面
- `popup.css`：界面样式
- `popup.js`：书签读取、添加、修改、删除、搜索、文件夹管理、导入导出、去重和实时刷新逻辑
- `icons/`：插件图标